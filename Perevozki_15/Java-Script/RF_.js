...
function loadStylesheet() {
    ...
    if (isMobile) {
        mainStylesheet.href = 'Mobile-css/mobile-RF_.css';
    } else {
        mainStylesheet.href = 'CSS/RF_.css';
    }
}
...
document.querySelectorAll('.route').forEach(route => {
    ...
    const driverName = drivers[randomDriverIndex];
    route.querySelector('.route-driver').innerText = driverName;
});

const HOVER_DELAY = 300;

document.querySelectorAll('.route-cities').forEach(cityElement => {
    let hoverTimeout;

    cityElement.addEventListener('mouseenter', () => {
        hoverTimeout = setTimeout(() => {
            const fullText = cityElement.innerText;
            cityElement.setAttribute('title', fullText);
            cityElement.innerText = fullText;
        }, HOVER_DELAY);
    });

    cityElement.addEventListener('mouseleave', () => {
        clearTimeout(hoverTimeout);
        const originalText = cityElement.getAttribute('data-original-text');
        if (originalText) {
            cityElement.innerText = originalText;
        }
    });

    const originalText = cityElement.innerText;
    cityElement.setAttribute('data-original-text', originalText);
});
...
const cities2 = [
    ...
    'Якутск', 'Янтарный', 'Ярославль', 'Выборг', 'Всеволожск', 'Великий Устюг', 'Великие Луки', 'Владикавказ', 'Владимир', 'Владивосток', 'Волгоград', 'Волгодонск', 'Волжский', 'Воркута', 'Голубицкая', 'Грязи', 'Грозный', 'Иноземцево', 'Кингисепп', 'Курово', 'Мышкин', 'Небуг', 'Никола', 'Нягань', 'Октябрьский', 'Партизанск', 'Петропавловск-камчатский', 'Пушкин', 'Фрязино', 'Электросталь'
];
...
function populateCityList() {
    ...
    fromList.innerHTML = fromCityOptions;
    toList.innerHTML = toCityOptions;
    fromList2.innerHTML = fromCityOptions;
    toList2.innerHTML = toCityOptions;
    fromList3.innerHTML = fromCityOptions;
    toList3.innerHTML = toCityOptions;
}
